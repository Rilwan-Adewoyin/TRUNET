...
def train_loop(train_params, model_params): 
    ...
    var_optimizer_step = tf.Variable( intial_value=int( batches_to_skip + (starting_epoch)*train_set_size_batches), trianable=False, name="iter", shape=[], dtype=tf.dtype.int64, aggregation=tf_variables.VariableAggregation.ONLY_FIRST_REPLICA )
    if tfa==None:
        ...
        if model_params['model_type_settings']['model_version'] in ["54","55","56","155"]:
            ...
            optimizer_rain      = tfa.optimizers.RectifiedAdam( **{"learning_rate":1.5e-3, "warmup_proportion":0.75, "min_lr":1e-3, "beta_1":0.05, "beta_2":0.85, "decay":0.004, "amsgrad":True, "epsilon":1e-3} , total_steps=total_steps ) 
            optimizer_nonrain   = tfa.optimizers.RectifiedAdam( **{"learning_rate":1e-3, "warmup_proportion":0.75, "min_lr":1e-3, "beta_1":0.05, "beta_2":0.85, "decay":0.004, "amsgrad":True,"epsilon":1e-3} , total_steps=total_steps ) 
            optimizer_dc        = tfa.optimizers.RectifiedAdam( **{"learning_rate":1.25e-3, "warmup_proportion":0.75, "min_lr":1e-3, "beta_1":0.05, "beta_2":0.85, "decay":0.004, "amsgrad":True,"epsilon":1e-3} , total_steps=total_steps )  
