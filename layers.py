...
class TRUNET_OutputLayer(tf.keras.layers.Layer):
	...
	def call(self, _inputs, training=True ):
		...
		if self.dc == False:
			...
			indexes1 = tf.range(start=0, limit=_inputs.shape[4]//3, dtype=tf.int32)
			indexes2 = tf.range(start=_inputs.shape[4]//3, limit=_inputs.shape[4], dtype=tf.int32)

			x_val = self.conv_hidden_val( self.do0( tf.gather(_inputs,indexes2,axis=4), training=training))
			x_prob = self.conv_hidden_prob( self.do1( tf.gather(_inputs,indexes1, axis=4),training=training))
