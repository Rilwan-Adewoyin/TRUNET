...
def main(m_params):
    ...
    f_testings =  [ open(f"hypertune/hypertune_test{idx+1}.txt","w") for idx in range(3) ]
            ...
            for b2 in b2s:
                for inpd in inp_dropouts:
                    for recd in rec_dropouts:
                        ...
                        # else:
                        #     train_cmd = train_cmd_maker( m_params['model_name'], lr, b1, b2, inpd, recd, counter,"3" )
                        #     f_training2.write(f'{train_cmd} && ')
...
def train_cmd_maker( mn ,lr_min_max, b1, b2, inp_drop, rec_drop, counter,gpu=None):
    cmd = [
        ...
        "-dd", "/media/Data3/akanni/Rain_Data_Mar20", "-bs", "63"]
